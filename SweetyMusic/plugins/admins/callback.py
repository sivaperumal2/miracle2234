...
from SweetyMusic import YouTube, app
from SweetyMusic.core.call import Sweety
from SweetyMusic.misc import SUDOERS, db
from SweetyMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from SweetyMusic.utils.decorators.language import languageCB
from SweetyMusic.utils.formatters import seconds_to_min
from SweetyMusic.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup)
from SweetyMusic.utils.stream.autoclear import auto_clean
from SweetyMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Sweety.pause_stream(chat_id)
        ...
        await Sweety.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Sweety.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Sweety.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Sweety.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Sweety.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
